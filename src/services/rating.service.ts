...
export const batchSettleLastMonth = async () => {
  ...
  console.log(
    `[Job] 开始结算: ${lastYear}-${lastMonth} -> ${thisYear}-${thisMonth}`,
  );
  ...
  for (const user of users) {
    ...
    try {
      ...
      console.log(
        `用户 ${user.realName}: 月初(${startTotal}) -> 月末(${currentTotal}) = 新增 ${increment}`,
      );
...
export const calculateRawScore = (
  ...
  const { BASE_SCORE, WEIGHTS, AWARD_DECAY, AWARD_WEIGHTS } =
    RATING_CONFIG.CONTEST;
  const currentSeason = getCurrentSeason();
...
// 结算某个月的刷题分 (并保存到数据库)
export const settleMonthlyPractice = async (
  userId: string,
  year: number,
  month: number,
  problemCount: number,
...
export const calculatePracticeRating = async (userId: string) => {
  ...
  return Math.min(
    RATING_CONFIG.PRACTICE.SEASON_MAX,
 ...
 */
export const calculateLegacyRating = async (
  userId: string,
  baseSeason?: string,
) => {
...
export const recalculateAllContestRatings = async (dryRun = false) => {
  await ensureContestRecordArchiveFields();

  const users = await User.find({ role: { $ne: "Teacher" } }).select(
    "_id realName ratingInfo",
  );
  const results: Array<{
    userId: string;
    realName: string;
  ...
  for (const user of users) {
    const newContest = await calculateContestRating(user._id.toString());
    const oldContest = user.ratingInfo?.contest || 0;
    const problem = user.ratingInfo?.problem || 0;
    const legacy = user.ratingInfo?.legacy || 0;
    const oldRating = user.rating || 0;
    const newRating = parseFloat((newContest + problem + legacy).toFixed(2));
 ...
 */
export const archiveAndResetSeason = async (
  oldSeason: string,
  newSeason: string,
) => {
  console.log(`[Season] 开始归档: ${oldSeason} -> ${newSeason}`);

  const users = await User.find({ role: { $ne: "Teacher" } }).sort({
    rating: -1,
  });
  let count = 0;
  ...
  for (let i = 0; i < users.length; i++) {
    ...
    try {
      ...
      const newLegacyScore = await calculateLegacyRating(
        user._id.toString(),
        newSeason,
      );
